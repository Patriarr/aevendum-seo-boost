...
export const ProductCard = ({ product, index }: ProductCardProps) => (
  <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1 }}
    className="bg-background rounded-lg border border-border hover-lift overflow-hidden group">
    <div className="relative aspect-square overflow-hidden bg-secondary/50">
      ...
      {product.badge && (
        <span className="absolute top-4 left-4 text-[10px] font-body font-semibold tracking-wider uppercase bg-foreground text-background px-3 py-1.5 rounded-sm">
          {product.badge}
        </span>
      )}
    </div>
    <div className="p-6">
      <p className="text-[10px] font-body tracking-[0.3em] uppercase text-primary font-semibold mb-1">{product.category}</p>
