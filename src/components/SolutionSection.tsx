  ...
  <section className="py-24 md:py-36 bg-secondary/30">
    <div className="container mx-auto px-4">
      <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-16">
        <p className="text-xs tracking-[0.3em] uppercase text-gold mb-4 font-body font-medium">La Solución Aevendum</p>
        <h2 className="font-display text-3xl md:text-5xl font-light text-foreground leading-tight">
          El Método <span className="gold-text font-medium">Regen-Cell</span>
        </h2>
      </motion.div>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-5xl mx-auto">
        {steps.map((step, i) => (
          <motion.div key={step.label} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.15 }}
            ...
            <div className="w-12 h-12 rounded-full flex items-center justify-center gold-gradient mb-5">
              <step.icon className="w-5 h-5 text-primary-foreground" />
            </div>
            <span className="text-[10px] font-body tracking-[0.3em] uppercase text-gold font-semibold">{step.protocol}</span>
            <h3 className="font-display text-xl font-semibold text-foreground mt-1 mb-1">{step.label}</h3>
